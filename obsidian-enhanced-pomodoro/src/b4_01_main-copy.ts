...
export default class EnhancedPomodoro extends Plugin {
  ...
  async onload() {
    ...
    // Initialize timeRemaining with default work duration
    ...
    // Initialize Kanban integration (always active)
  ...
  async onunload() {
    // Clean up Kanban integration if it was set up
    this.removeKanbanIntegration();
...
class EnhancedPomodoroSettingTab extends PluginSettingTab {
  ...
  display(): void {
    ...
    const refreshKanbanList = async () => {
      ...
      debugLog.push(`## Kanban File Detection Debug - ${new Date().toISOString()}`);
