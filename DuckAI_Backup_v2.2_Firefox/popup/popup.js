...
const FILENAME_FORMATS = {
  ...
  format4: {
    name: "Simple",
    template: (index, title, backupType, date) => `${title}_${date}.md`,
...
async function generateFilename(index, title, backupType, date) {
  ...
  const format =
    FILENAME_FORMATS[formatKey] || FILENAME_FORMATS[DEFAULT_FILENAME_FORMAT];
...
// NEW v2.1: HYBRID extraction - localStorage + HTML sidebar fallback
// Duck.ai only stores ~6 recent chats in localStorage, older ones are in sidebar HTML only
function extractFromLocalStorage() {
  console.log("=== HYBRID EXTRACTION (v2.1) ===");
  ...
  // Format messages into readable markdown content
  function formatMessages(messages) {
    ...
    return messages
      .map((msg) => {
        ...
        // Ensure content is a string (could be null, undefined, or object)
        let content = "";
        if (typeof msg.content === "string") {
          content = msg.content;
        } else if (msg.content && typeof msg.content === "object") {
          // Handle case where content is an object (e.g., image data)
          content = msg.content.text || JSON.stringify(msg.content);
        }

        // Handle parts array (tool invocations, text parts)
  ...
  // Extract chat titles from HTML sidebar (for chats not in localStorage)
  function extractSidebarTitles() {
    const sidebarTitles = [];
  ...
  try {
    const results = [];
    const localStorageTitles = new Set();
    const messageFingerprints = new Map(); // NEW v2.1: Track conversations by message content

    // STEP 1: Extract from localStorage (full content)
    const savedChatsRaw = localStorage.getItem("savedAIChats");

    if (savedChatsRaw) {
      const savedChats = JSON.parse(savedChatsRaw);
      console.log("savedAIChats version:", savedChats.version);
      console.log("Number of chats in localStorage:", savedChats.chats?.length || 0);

      if (savedChats.chats && savedChats.chats.length > 0) {
        console.log(`📦 localStorage has ${savedChats.chats.length} conversations with FULL content`);
        console.log("📝 Titles in localStorage:", savedChats.chats.map((c, i) => `${i + 1}. "${c.title?.substring(0, 40)}..."`).join("\n"));
        ...
        for (let i = 0; i < savedChats.chats.length; i++) {
          ...
          // Format all messages into markdown content
          const content = formatMessages(messages);

          // Get first message date for metadata
          const firstMessageDate = messages[0]?.createdAt || new Date().toISOString();
          const lastMessageDate = messages[messages.length - 1]?.createdAt || firstMessageDate;

          const contentHash = generateContentHash(title, content);

          // Create fingerprint from first few messages (survives title rename)
          const fingerprint = messages.slice(0, 2).map(m => {
          ...
          results.push({
            ...
            fingerprint: fingerprint, // NEW v2.1
          });

          localStorageTitles.add(title.toLowerCase().trim());
          if (fingerprint) {
            messageFingerprints.set(fingerprint, results.length - 1);
          }

          console.log(
            `✅ [localStorage] Chat ${i + 1}: "${title.substring(0, 50)}..." (${messages.length} messages, ${content.length} chars)`,
          );
        }
      }
    } else {
      console.warn("No savedAIChats found in localStorage");
    }

    // STEP 2: Check HTML sidebar for additional chats NOT in localStorage
    const sidebarTitles = extractSidebarTitles();
    let sidebarOnlyCount = 0;
    let renamedMatchCount = 0;

    console.log(`\n📋 Sidebar has ${sidebarTitles.length} total conversation titles`);
    console.log(`🔍 Checking for renamed conversations and missing content...`);

    for (const sidebarTitle of sidebarTitles) {
      const normalizedTitle = sidebarTitle.toLowerCase().trim();
      
      // Skip if already extracted from localStorage (exact title match)
      if (localStorageTitles.has(normalizedTitle)) {
        console.log(`✅ [EXACT MATCH] "${sidebarTitle.substring(0, 40)}..." found in localStorage`);
        continue;
      }

      // NEW v2.1.1: Check if this might be a RENAMED conversation
      // Try to match by checking if sidebar title words appear in any localStorage conversation
      let foundMatch = false;
      const sidebarWords = normalizedTitle.split(/\s+/).filter(w => w.length > 3);
      ...
      if (sidebarWords.length >= 2) { // Lowered threshold from 3 to 2
        for (let i = 0; i < results.length; i++) {
          ...
          const resultContentPreview = result.content.substring(0, 500).toLowerCase();
          
          // Strategy 1: Check if sidebar title words appear in conversation content
          const matchCount = sidebarWords.filter(word => 
            resultContentPreview.includes(word)
          ).length;
          
          // Strategy 2: Check if localStorage title words appear in new sidebar title
          const resultWords = resultTitleLower.split(/\s+/).filter(w => w.length > 3);
          const reverseMatchCount = resultWords.filter(word =>
      ...
      if (foundMatch) {
        continue; // Already matched to localStorage conversation
      }
    ...
    if (renamedMatchCount > 0) {
      console.log(`\n🔄 Successfully matched ${renamedMatchCount} renamed conversations!`);
    }

    if (results.length === 0) {
      return {
        error: true,
        message: "No saved chats found. Make sure you have conversations on duck.ai",
      };
    }

    console.log(
      `=== SUCCESS: Extracted ${results.length} conversations (${results.length - sidebarOnlyCount} full, ${sidebarOnlyCount} sidebar-only) ===`,
    );
    return results;
  } catch (error) {
...
// LEGACY: Keep old HTML-based function as fallback (renamed)
async function backupConversationsLegacy(progressCallback) {
  console.log("=== LEGACY HTML EXTRACTION (fallback) ===");
  console.log("Current URL:", window.location.href);
  ...
  console.log(
    `After smart filtering: ${conversationElements.length} conversation items identified`,
  );

  console.log(
    `Found ${conversationElements.length} conversation elements total`,
  );
  ...
  if (mainElement) {
    ...
    for (const selector of messageSelectors) {
      ...
      if (messages.length > 0) {
        console.log(
          `Found ${messages.length} message elements using selector: ${selector}`,
        );
        ...
        if (combinedText.length > 100) {
          ...
          console.log(
            `✅ Extracted ${currentConversationContent.length} chars from ${messages.length} messages`,
          );
          break;
        }
      }
    ...
    if (!currentConversationContent) {
      ...
      if (mainText.length > 100) {
        ...
        console.log(
          `✅ Extracted ${currentConversationContent.length} chars from <main> element (fallback)`,
        );
      }
    }
  } else {
  ...
  console.log(
    `Final extracted content length: ${currentConversationContent.length} chars`,
  );
    ...
    if (title.length > 5 && !title.toLowerCase().includes("button")) {
      ...
      try {
        ...
        if (currentConversationContent.length > 100) {
          ...
          const contentStart = currentConversationContent
            .toLowerCase()
            .substring(0, 100);
          ...
          if (contentStart.includes(titleStart) || index === 0) {
            ...
            console.log(
              `✅ Using main conversation content for "${title.substring(0, 30)}..." (${conversationContent.length} chars)`,
            );
          }
        }
        ...
        if (!conversationContent) {
          ...
          console.log(
            `⚠️ Using sidebar text for "${title.substring(0, 30)}..." (${conversationContent.length} chars) - limited content`,
          );
        }
        ...
        if (!conversationContent || conversationContent.length < 10) {
          ...
          console.log(
            `⚠️ Using title only for "${title.substring(0, 30)}..." - NO CONTENT AVAILABLE`,
          );
        }
        ...
        console.log(
          `✅ Added conversation ${index + 1}: "${title.substring(0, 50)}..." (content: ${conversationContent.length} chars, source: ${contentSource}, total results: ${results.length})`,
        );
        ...
        if (progressCallback) {
          progressCallback(
            index + 1,
            conversationElements.length,
            `Processed ${index + 1}/${conversationElements.length}`,
          );
        }
      } catch (error) {
        console.error(
          `Error creating conversation object for ${index + 1}:`,
          error,
        );
      }
    } else {
      console.log(
        `❌ Skipped element ${index + 1} - failed validation (length: ${title.length}, contains 'button': ${title.toLowerCase().includes("button")})`,
      );
    }
  }
...
async function backupConversationsWithAutoClick(progressCallback) {
    ...
    for (const selector of messageSelectors) {
      ...
      if (messages.length > 0) {
        ...
        if (combinedText.length > 100) {
          ...
          console.log(
            `Extracted ${content.length} chars using selector: ${selector}`,
          );
          break;
        }
      }
  ...
  const allDivsWithTitle = Array.from(document.querySelectorAll("div[title]"));
  console.log(
    `[Full Backup] Found ${allDivsWithTitle.length} total div[title] elements`,
  );
  ...
  const conversationElements = allDivsWithTitle.filter((el) => {
    ...
    // If we got here, it's likely a conversation
    console.log(
      `[Full Backup] ✅ Accepting conversation: "${title.substring(0, 50)}..."`,
    );
    return true;
  });

  console.log(
    `[Full Backup] After smart filtering: ${conversationElements.length} conversation items identified`,
  );
  ...
  console.log(
    `Will process ${conversationElements.length} conversations with auto-clicking`,
  );
  ...
  for (let i = 0; i < conversationElements.length; i++) {
    ...
    if (progressCallback) {
      progressCallback(
        i,
        conversationElements.length,
        `Clicking conversation ${i + 1}/${conversationElements.length}...`,
      );
    }
    ...
    try {
      ...
      if (titleWords && !contentLower.includes(titleWords.substring(0, 20))) {
        console.warn(
          `⚠️ Content mismatch warning: Title words not found in content. This may be content bleeding!`,
        );
        console.warn(`   Expected: "${titleWords.substring(0, 30)}..."`);
        console.warn(
          `   Got content preview: "${content.substring(0, 100)}..."`,
        );
      }
      ...
      results.push(conversationItem);
      console.log(
        `✅ Added to results (${results.length}/${conversationElements.length})`,
      );
      ...
      if (progressCallback) {
        progressCallback(
          i + 1,
          conversationElements.length,
          `Processed ${i + 1}/${conversationElements.length} conversations`,
        );
      }
    } catch (error) {
    ...
    func: function () {
      function fetchContents() {
        ...
        Array.from(document.querySelectorAll("h1,h2,h3")).forEach((heading) => {
          ...
          while (
            nextElement &&
            !["H1", "H2", "H3"].includes(nextElement.tagName)
          ) {
...
async function renderTitlesOnly() {
  ...
  if (!items || items.length === 0) {
    list.innerHTML =
      '<li style="color: #666; font-style: italic;">No items to display</li>';
    return;
  }
  ...
  console.log(
    `[renderTitlesOnly] Found ${downloadedFiles.length} downloaded files in history`,
  );
  ...
  for (let index = 0; index < items.length; index++) {
    ...
    if (conversationData && conversationData[item.index]) {
      ...
      if (isTitlesOnly) {
        ...
        console.log(
          `[renderTitlesOnly] Status for "${conversation.title.substring(0, 30)}": ${status?.icon || "none"}`,
        );
      }
    }
...
refreshBtn.addEventListener("click", async () => {
  try {
    ...
    console.log(
      "✅ Full Backup Mode checkbox set to:",
      document.getElementById("fullBackupMode").checked,
    );
    ...
    if (!conversationData || conversationData.length === 0) {
      ...
      try {
        ...
        console.error(
          "Expected URL pattern: https://duckduckgo.com/?*duckai=1*",
        );
      } catch (e) {
    ...
    console.log(
      `✅ Full Backup loaded ${conversationData.length} conversations:`,
    ...
    // Verify we got full content (localStorage or main_content are both full)
    const fullContentCount = conversationData.filter(
      (c) =>
        c.contentSource === "main_content" ||
        c.contentSource === "localStorage",
    ).length;
    console.log(
      `📊 Content sources: ${fullContentCount} full, ${conversationData.length - fullContentCount} partial`,
    );

    if (fullContentCount === 0) {
      statusEl.textContent =
        "⚠️ No full content loaded - try refreshing the page";
      hideProgress();
      return;
    }
    ...
    console.log(
      `📋 Rebuilt items array with ${items.length} items, all selected`,
    );
    ...
    for (let i = 0; i < checkedItems.length; i++) {
      ...
      if (conversation) {
        console.log(
          `📥 Downloading ${i + 1}/${checkedItems.length}:`,
          conversation.title,
        );
        updateProgress(
          i + 1,
          checkedItems.length,
          `Downloading: ${conversation.title.substring(0, 30)}...`,
        );

        const result = await downloadSingleConversation(
          conversation,
          i,
          checkedItems.length,
        );
        console.log(
          `✅ Download complete for "${conversation.title}":`,
          result,
        );
    ...
    await addToBackupHistory(
      checkedItems.map((item) => conversationData[item.index]),
    );

    // Clear cache after backup
    console.log(
      "Clearing cache after backup for accurate status on next load...",
    );
    await browser.storage.local.remove([CACHE_KEY, CACHE_KEY + "_timestamp"]);
    usingCachedData = false;
...
async function loadTitlesOnly() {
  ...
  try {
    ...
    // No cache - extract from localStorage (v2.1 - much faster and more reliable!)
    statusEl.textContent = "Extracting conversations from localStorage...";
    showProgress();
    ...
    updateProgress(30, 100, "Reading localStorage...");

    const results = await browser.scripting.executeScript({
      target: { tabId: tab.id },
      func: extractFromLocalStorage, // NEW v2.1: localStorage extraction
    });

    updateProgress(80, 100, "Processing conversations...");
    updateProgress(100, 100, "Complete!");

    // Check if script executed successfully
    if (!results || !results[0] || !results[0].result) {
      hideProgress();
      statusEl.textContent = "⚠️ Not on duck.ai - Please navigate to duck.ai";
      conversationData = [];
      items = [];
      await renderTitlesOnly();
      return;
...
// v2.1: Simplified loadConversations using localStorage extraction
// No more Full Backup Mode toggle needed - localStorage always has full content!
async function loadConversations() {
  console.log("🔍 loadConversations() v2.1 - localStorage extraction");

  statusEl.textContent = "Loading conversations from localStorage...";
  showProgress();
  ...
  try {
    ...
    if (cachedData && Array.isArray(cachedData) && cachedData.length > 0) {
      ...
      hideProgress();
      statusEl.textContent = `✓ Loaded ${cachedData.length} conversations (cached, ${cacheInfo.remainingMinutes}min left)`;
      await renderTitlesOnly();
      return;
    }

    // Fresh extraction from localStorage
    usingCachedData = false;
    updateProgress(30, 100, "Reading localStorage...");
    ...
    console.log("localStorage extraction result:", result);

    // Check for errors
    if (result && result.error) {
      hideProgress();
      statusEl.textContent = `⚠️ ${result.message || "No conversations found"}`;
      conversationData = [];
      items = [];
      await renderTitlesOnly();
      return;
    ...
    if (
      ...
      hideProgress();
      statusEl.textContent = "No conversations found in localStorage";
      conversationData = [];
      items = [];
      await renderTitlesOnly();
      return;
    ...
    // Count message stats
    const totalMessages = conversationData.reduce(
      (sum, c) => sum + (c.messageCount || 0),
      0,
    );
    statusEl.textContent = `✅ Loaded ${conversationData.length} conversations (${totalMessages} messages total)`;
    ...
    if (
      ...
      error.message?.includes("scripting")
    ) {
      errorMsg = "Permission denied - Make sure you are on duck.ai";
...
document
  ...
  .addEventListener("change", async function () {
    ...
    statusEl.textContent =
      "⚠️ Mode changed - You must reload conversations before downloading!";
  });
...
toggleBtn.addEventListener("click", async () => {
  if (items.length === 0) {
    statusEl.textContent =
      '⚠️ No conversations loaded yet. Click "Load Chat-titles" first.';
    return;
  }
...
function htmlToMarkdown(html) {
  ...
  result = result.replace(
    ...
    (match, preContent) => {
      ...
      const languageMatch = preContent.match(
        /<p[^>]*class="[^"]*li3asHIMe05JPmtJCytG[^"]*"[^>]*>(.*?)<\/p>/i,
      );
  ...
  result = result.replace(
    ...
    (match, tableContent) => {
      ...
      // Extract header row
      const headerMatch = tableContent.match(
        /<thead[^>]*>([\s\S]*?)<\/thead>/i,
      );
      if (headerMatch) {
        const headerCells = [];
...
async function checkLocalDownloadedFiles() {
  try {
    const downloadedFiles = (await browser.storage.local.get([
      "downloadedFiles",
    ])) || { downloadedFiles: [] };
...
async function checkDuplicateConversation(conversation) {
  try {
    const downloadedFiles = await checkLocalDownloadedFiles();
    const duplicate = downloadedFiles.find(
      (file) => file.conversationHash === conversation.contentHash,
    );
    ...
    if (duplicate) {
      return {
        ...
        wasFullBackup:
          duplicate.contentSource === "main_content" ||
          duplicate.contentSource === "localStorage", // NEW: Flag for full backups
      };
    }
...
async function getBackupStatus(conversation) {
  ...
  if (syncResult.icon === "✅") {
    ...
    backupType = canUpgrade
      ? "sidebar_text"
      : syncResult.label.includes("Full")
        ? "main_content"
        : "sidebar_text";
...
function getBackupStatusSync(conversation, downloadedFiles) {
  try {
    ...
    const hashMatches = downloadedFiles.filter(
      (f) => f.conversationHash === currentHash,
    );
    const exactMatch =
      hashMatches.length > 0
        ? hashMatches.reduce((latest, current) =>
    ...
    if (exactMatch) {
      const isFullBackup =
        exactMatch.contentSource === "main_content" ||
        exactMatch.contentSource === "localStorage";
      return {
        icon: isFullBackup ? "✅" : "⚠️",
        label: isFullBackup
    ...
    const shouldCheckOutdated =
      !usingCachedData &&
      (currentSource === "main_content" || currentSource === "localStorage") &&
      conversation.content &&
      conversation.content.length > 200 && // Require substantial content
      downloadedFiles &&
      downloadedFiles.length > 0; // Require backup history loaded

    // IMPORTANT: Filter all potential matches first, then find the MOST RECENT outdated backup
    const outdatedMatches = shouldCheckOutdated
      ? downloadedFiles.filter((f) => {
          if (f.title !== title || f.contentSource !== currentSource)
            return false;
          ...
          if (backupLength > 0) {
            ...
            if (percentDiff < 10) {
              console.log(
                `[Status Check] Similar length (${percentDiff.toFixed(1)}% diff) - treating as unchanged: "${title.substring(0, 30)}"`,
              );
              return false;
            }

            console.log(
              `[Status Check] Significant length change (${percentDiff.toFixed(1)}% diff) - marking as outdated: "${title.substring(0, 30)}"`,
            );
          }
    ...
    const outdatedMatch =
      outdatedMatches.length > 0
        ? outdatedMatches.reduce((latest, current) =>
            current.timestamp > latest.timestamp ? current : latest,
          )
        : null;
    ...
    const sameTitleMatches = downloadedFiles.filter(
      (f) => f.title === title && f.contentSource === currentSource,
    );
    const sameTitle =
      sameTitleMatches.length > 0
        ? sameTitleMatches.reduce((latest, current) =>
            current.timestamp > latest.timestamp ? current : latest,
          )
        : null;
    ...
    if (sameTitle) {
      ...
      const isFullBackup =
        sameTitle.contentSource === "main_content" ||
        sameTitle.contentSource === "localStorage";
      return {
        icon: isFullBackup ? "✅" : "⚠️",
        label: isFullBackup
    ...
    const mostRecentBackup =
      allBackupsForTitle.length > 0
        ? allBackupsForTitle.reduce((latest, current) =>
            current.timestamp > latest.timestamp ? current : latest,
          )
        : null;
    ...
    if (
      ...
      mostRecentBackup.contentSource !== "localStorage" &&
      (currentSource === "main_content" || currentSource === "localStorage")
    ) {
      return {
        icon: "⚠️",
...
async function addToBackupHistory(items) {
  try {
    const { [BACKUP_HISTORY_KEY]: history = [] } =
      await browser.storage.local.get([BACKUP_HISTORY_KEY]);
...
async function downloadSingleConversation(conversation, index, total) {
  ...
  // Determine backup type label for filename (localStorage = full content)
  const isFullContent =
  ...
  const obsidianFrontmatter =
    document.getElementById("obsidianFrontmatter")?.checked ?? true;
  ...
  if (obsidianFrontmatter) {
    markdownContent += generateObsidianFrontmatter(
      conversation,
      index,
      backupType,
    );
  ...
  if (duplicateCheck.isDuplicate) {
    const previousBackupType =
      duplicateCheck.contentSource === "main_content" ||
      duplicateCheck.contentSource === "localStorage"
        ? "✅ FULL CONTENT"
        : "⚠️ PARTIAL CONTENT";
    const currentBackupType = isFullContent
      ? "✅ FULL CONTENT"
      : "⚠️ PARTIAL CONTENT";
    ...
    if (!duplicateCheck.wasFullBackup && isFullContent) {
  ...
  if (contentSource === "sidebar_text") {
    ...
    markdownContent += `**✅ FULL CONTENT**: This backup contains the complete conversation.\n\n`;
  }
  ...
  if (conversation.html && conversation.html.length > content.length * 0.5) {
    ...
    console.log(
      `Using HTML content for better formatting (${conversation.html.length} chars)`,
    );
  }
  ...
  try {
    ...
    try {
      ...
      console.warn(
        "⚠️ Folder path failed, trying filename only:",
        folderError.message,
      );
      downloadId = await browser.downloads.download({
        url: url,
  ...
  if (!duplicateCheck.isDuplicate || isUpgrade) {
    ...
    if (isUpgrade) {
      console.log(
        `✅ Backup upgraded from ${duplicateCheck.contentSource} to ${contentSource}`,
      );
    }
  }
...
document
  ...
  .addEventListener("click", async () => {
    ...
    if (!fullBackupMode) {
      const partialCount = conversationData.filter(
        (c) =>
          c.contentSource !== "main_content" &&
          c.contentSource !== "localStorage",
      ).length;
      if (partialCount > 0) {
        ...
        if (!proceed) {
          statusEl.textContent =
            "Quick download cancelled - use Full Backup for complete content";
          return;
        }
      }
    }
    ...
    if (conversationData.length > 0) {
      const hasFullContent = conversationData.some(
        (c) =>
          c.contentSource === "main_content" ||
          c.contentSource === "localStorage",
      );
      const hasPartialContent = conversationData.some(
        (c) =>
          c.contentSource !== "main_content" &&
          c.contentSource !== "localStorage",
      );
    ...
    try {
      ...
      await addToBackupHistory(
        finalCheckedItems.map((item) => conversationData[item.index]),
      );
      ...
      const duplicateCount = downloadResults.filter(
        (r) => r.isDuplicate,
      ).length;
      const upgradeCount = downloadResults.filter((r) => r.isUpgrade).length;
      const newCount = downloadResults.length - duplicateCount - upgradeCount;
      const fullContentCount = downloadResults.filter(
        (r) => r.contentSource === "main_content",
      ).length;
      const limitedContentCount = downloadResults.filter(
        (r) => r.contentSource === "sidebar_text",
      ).length;
      const titleOnlyCount = downloadResults.filter(
        (r) => r.contentSource === "title_only",
      ).length;
      ...
      statusEl.textContent =
        "Error during download - check console for details";
    }
  });

// NEW: Enhanced Backup Status Viewer with Outdated Detection
document
  .getElementById("viewBackupStatus")
  .addEventListener("click", async () => {
    try {
      // AUTO-LOAD if no conversations are loaded yet
      if (!conversationData || conversationData.length === 0) {
      ...
      const legacyBackups = downloadedFiles.filter(
        (f) => !f.contentSource,
      ).length;
      ...
      <div style="position: fixed; top: 0; left: 0; width: 100vw; height: 100vh; background: rgba(0,0,0,0.6); z-index: 1000; display: flex; align-items: center; justify-content: center;">
        <div style="background: white; border-radius: 12px; padding: 25px; max-width: 650px; max-height: 80vh; overflow-y: auto; box-shadow: 0 8px 32px rgba(0,0,0,0.3);">
          ...
          <div style="background: #e8f5e9; padding: 12px; border-radius: 8px; margin-bottom: 15px;">
            ...
            <div style="display: flex; gap: 20px; flex-wrap: wrap;">
              ...
              <div style="display: flex; align-items: center; gap: 6px;">
                ...
                <span style="font-weight: 500; color: #1565c0;">${autoBackupCount}</span>
                <span style="color: #666; font-size: 0.9em;">Auto-Backups</span>
              </div>
              ` : ''}
              ${manualImportCount > 0 ? `
              <div style="display: flex; align-items: center; gap: 6px;">
                <span style="font-size: 1.2em;">📥</span>
                <span style="font-weight: 500; color: #e65100;">${manualImportCount}</span>
                <span style="color: #666; font-size: 0.9em;">Manual Imports</span>
              </div>
              ` : ''}
            </div>
          </div>
          ...
          ${
            conversationData &&
            conversationData.length > 0 &&
            conversationData[0].contentSource !== "main_content"
              ? `
          ...
          <div style="max-height: 300px; overflow-y: auto;">
            ${conversationStatuses
              ...
              .map(({ conversation, status }) => {
                ...
                if (status.status === "up-to-date") {
                  statusLabel =
                    status.backupType === "main_content"
                      ? "Up to Date"
                      : "Partial";
                } else if (status.status === "outdated") {
                  statusLabel = "Updated";
...
function renderHistoryList(searchTerm = "") {
  ...
  if (fullHistory.length === 0) {
    ...
    p.textContent = 'No backup history found.';
  ...
  if (filtered.length === 0) {
    ...
    p.style.color = '#666';
    p.style.fontStyle = 'italic';
    p.textContent = `No matches found for "${searchTerm}"`;
    historyList.appendChild(p);
    return;
  }
  ...
  historyList.innerHTML = filtered
    .map((item, index) => {
      ...
      const remainingCount = searchTerm
        ? 0
        : Math.max(0, item.items.length - 3);

      return `<div style="border-bottom: 1px solid #eee; padding: 8px 0;">
      <strong>Backup ${fullHistory.indexOf(item) + 1}</strong> - ${escapeHTML(new Date(item.timestamp).toLocaleString())}<br>
      ...
      <div style="margin-top: 4px; font-size: 0.8em; max-height: ${searchTerm ? "200px" : "60px"}; overflow-y: auto;">
        ${displayItems
          .map((conv) => {
            const typeIcon =
              conv.contentSource === "main_content"
            ...
            const safeTitle = escapeHTML(conv.title || 'Untitled');
            const truncated = conv.title && conv.title.length > 50 ? safeTitle.substring(0, 50) + '...' : safeTitle;
            return `<div style="color: #888;">• ${typeIcon} ${truncated}</div>`;
          })
          .join("")}
...
if (debugSettingsBtn) {
  debugSettingsBtn.addEventListener("click", async () => {
    ...
    console.log("=== ALL STORED SETTINGS ===", allSettings);
